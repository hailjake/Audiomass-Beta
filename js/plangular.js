            ...
            plangular.directive('plangular', ['$timeout', '$rootScope', 'plangularConfig', function ($timeout, $rootScope, plangularConfig) {
                ...
                return {
                    ...
                    link: function (scope, elem, attr) {
                        ...
                        $rootScope.$on("addSongFromQueue", function (evt, data) {
                            if (data) {
                                scope.tracks.push(data);
                            }
                        });
                        $rootScope.$on("pauseSongFromQueue", function (evt, data) {
                        ...
                        scope.index = 0;
                        scope.playlist;
                        scope.repeat = attr.plangularRepeat;
                        scope.tracks = [];
          ...
          'Register for app at https://developers.soundcloud.com/',
                        ...
                        scope.play = function (i) {
                            if (typeof i !== 'undefined' && scope.tracks.length) {
                                scope.index = i;
                                scope.track = setSrc(scope.tracks[i], true);
                            } else {
